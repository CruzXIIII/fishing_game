...
def get_random_fish(player):
    ...
    # Calculate total luck using base rod luck and player's custom multiplier
    base_luck = rod.get("luck", 1)
    luck_mult = getattr(player, 'custom_luck_multiplier', 1.0)
    total_luck = base_luck * luck_mult

    # Apply luck floor to ensure a non-zero chance for all rarities, scaling with total_luck
    # Example floors: Rare: luck * 0.001, Legendary: luck * 0.0005, Mythical: luck * 0.0002, Exotic: luck * 0.0001, Secret: luck * 0.00005
    luck_floors = [0.0, 0.0, 0.001, 0.0005, 0.0002, 0.0001, 0.00005]
    for i in range(7):
        floor_value = total_luck * luck_floors[i]
        if base_weights[i] < floor_value:
            base_weights[i] = floor_value
